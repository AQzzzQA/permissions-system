...
async function migrate() {
    ...
    `);
        ...
        for (const permission of defaultPermissions) {
            await connection.execute('INSERT IGNORE INTO skill_permissions (id, role_id, action, scope) VALUES (?, ?, ?, ?)', [require('uuid').v4(), permission.role_id, permission.action, permission.scope]);
        }
        ...
        for (const skill of defaultSkills) {
            await connection.execute(`INSERT IGNORE INTO skills 
